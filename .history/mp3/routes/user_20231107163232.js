...
module.exports = function (router) {
    ...
    userRoute.get(function (req, res) {
        ...
        if(count) {
            User.find(where).sort(sort).select(select).skip(skip).limit(limit).countDocuments().exec().then(function(data) {
            ...
            User.find(where).sort(sort).select(select).skip(skip).limit(limit).exec().then(function(data) {
                return res.status(200).send({
                    message: 'users found',
                    data: data
                })
