...
module.exports = function(router) {
    ...
    userIdRoute.put(async function(req, res) {
       ...
       if(req.body.pendingTasks) {
        ...
        User.findByIdAndUpdate(userId, {$set:{updateInfo}}, { new: true }).exec().then(async function(userData) {
            ...
            if (userData == null) {
                ...
                try {
                    await Promise.all(userData.pendingTasks.map(newTask => {
                        return Tasks.findByIdAndUpdate(newTask, { assignedUser: userId, assigendUserName: userData.name }).exec();
                    }));
