...
module.exports = function(router) {
    ...
    tasksRoute.post(async function (req, res) {
        ...
        if(!('deadline' in req.body)) {
            ...
            newTasks.deadline = JSON.parse(req.body.deadline);
        }
        newTasks.description = req.body.description || '';
        newTasks.completed = JSON.parse(req.body.completed) || false;
        newTasks.dateCreated = req.body.dateCreated;
        if(!isValidObjectId(req.body.assignedUser)) {
        ...
        try {
            userData = await User.findOne({"name":assigned});
