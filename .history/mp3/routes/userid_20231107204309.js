...
module.exports = function(router) {
    ...
    userIdRoute.put(async function(req, res) {
       ...
       if(req.body.pendingTasks) {
        ...
        User.findByIdAndUpdate(userId, updateInfo, { new: true }).exec().then(async function(userData) {
            console.log(userId);
            ...
            if (userData == null) {
                ...
                if(updateInfo.pendingTasks) {
                    updateInfo.pendingTasks.forEach(function(newTask) {
                        Tasks.findById(newTask, {$set : {assignedUser:userId,assignedUserName:userData.name}}).exec().catch(err) {
                            return res.status(500).send({
