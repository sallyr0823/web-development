...
module.exports = function(router) {
    ...
    userIdRoute.put(async function(req, res) {
       ...
       if(req.body.pendingTasks) {
        ...
        try {
            ...
            oriData.pendingTasks.forEach(function(t) {
                if(!updateInfo.pendingTasks.includes(t)) {
                    updateInfo.pendingTasks.push(t);
                }
            })
            await Tasks.updateMany(
                {"_id": {$in:oriData.pendingTasks}},
            {$set:{"assignedUser":"","assignedUserName":updateInfo.name}}
            );
        } catch(error) {
            return res.status(500).send({
    ...
    userIdRoute.delete(function(req, res) {
        var userId = req.params.id;
        console.log(userId);
        User.findByIdAndDelete(userId).exec().then(async function(data) {
            console.log(data);
            if(data == null) {
                return res.status(404).send({
                    message: 'user not found',
            ...
            if(data.pendingTasks != null){
                try{
                    await Promise.all(data.pendingTasks.map(task => {
                        //console.log(task);
                        return Tasks.findByIdAndUpdate(task, {$set :{ 'assignedUser': "", 'assignedUserName': 'unassigned' }},{new:true}).exec();
                }));
                } catch(err) {
                    return res.status(500).send({
                        message: 'Error while updating tasks',
