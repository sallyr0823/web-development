...
module.exports = function(router) {
    ...
    userIdRoute.put(async function(req, res) {
       ...
       if(req.body.pendingTasks) {
        ...
        User.findOneAndUpdate({_id:userId}, {$set:{updateInfo}}, { new: true ,useFindAndModify:true}).exec().then(function(userData) {
        ...
        try {
            await Promise.all(updateInfo.pendingTasks.map(newTask => {
                return Tasks.findByIdAndUpdate(newTask, { assignedUser: userId, assigendUserName: userName }).exec();
            }));
